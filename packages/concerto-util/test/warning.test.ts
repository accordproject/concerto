...
import { printDeprecationWarning } from '../src/warning';
...
describe('printDeprecationWarning', () => {
    it('should emit the correct deprecation warning message', () => {
        ...
        process.once('warning', (warning: Error & { code?: string; detail?: string }) => {
            expect(warning.message).to.equal(`DEPRECATED: ${message}`);
            expect(warning.code).to.equal(code);
            expect(warning.detail).to.equal(detail);
        });
