...
class MetaModel {
    ...
    */
    static enumPropertyToMetaModel(ast) {
        const property = {};
    ...
    */
    static decoratorArgToMetaModel(ast) {
        const decoratorArg = {};
        switch (ast.type) {
        case 'String':
    ...
    */
    static decoratorToMetaModel(ast) {
        const decorator = {
        ...
        if (ast.arguments && ast.arguments.list) {
            if (!ast.arguments.list[0]) {
                ...
                decorator.arguments = ast.arguments.list.map(MetaModel.decoratorArgToMetaModel);
            }
        }
        return decorator;
    }
    ...
    */
    static decoratorsToMetaModel(ast) {
        return ast.map(MetaModel.decoratorToMetaModel);
    }
    ...
    * @return {object} the metamodel for this property
    */
    static propertyToMetaModel(ast) {
        const property = {};
        ...
        if (ast.decorators && ast.decorators.length > 0) {
            property.decorators = MetaModel.decoratorsToMetaModel(ast.decorators);
        }
    ...
    */
    static relationshipToMetaModel(ast) {
        let relationship = {
    ...
    */
    static enumDeclToMetaModel(ast) {
        let decl = {};
        ...
        for (let n = 0; n < ast.body.declarations.length; n++) {
            ...
            decl.properties.push(MetaModel.enumPropertyToMetaModel(thing));
        }
    ...
    */
    static conceptDeclToMetaModel(ast) {
        let decl = {};
        ...
        // Handle decorators
        if (ast.decorators && ast.decorators.length > 0) {
            decl.decorators = MetaModel.decoratorsToMetaModel(ast.decorators);
        }
        ...
        for (let n = 0; n < ast.body.declarations.length; n++) {
            ...
            if (thing.type === 'FieldDeclaration') {
                decl.properties.push(MetaModel.propertyToMetaModel(thing));
            } else if (thing.type === 'RelationshipDeclaration') {
                decl.properties.push(MetaModel.relationshipToMetaModel(thing));
            }
        }
    ...
    */
    static declToMetaModel(ast) {
        if(ast.type === 'EnumDeclaration') {
            return MetaModel.enumDeclToMetaModel(ast);
        }
        return MetaModel.conceptDeclToMetaModel(ast);
    }
    ...
    */
    static modelToMetaModel(ast, validate = true) {
        const metamodel = {
        ...
        for(let n=0; n < ast.body.length; n++ ) {
            ...
            const decl = MetaModel.declToMetaModel(thing);
    ...
    */
    static modelFileToMetaModel(modelFile, validate) {
        return MetaModel.modelToMetaModel(modelFile.ast, validate);
    }
    ...
    */
    static modelManagerToMetaModel(modelManager, resolve, validate) {
        ...
        modelManager.getModelFiles().forEach((modelFile) => {
            let metaModel = MetaModel.modelToMetaModel(modelFile.ast, validate);
            if (resolve) {
    ...
    */
    static ctoToMetaModel(model, validate) {
        ...
        return MetaModel.modelToMetaModel(ast);
    }
    ...
    */
    static ctoToMetaModelAndResolve(modelManager, model, validate) {
        const ast = parser.parse(model);
        const metaModel = MetaModel.modelToMetaModel(ast);
