...
const debug = require('debug')('concerto:LoadModule');
...
class LoadModule{
    ...
    static loadModule(nameToLoad,options = {}) {
        ...
        debug('loadModule',`${nameToLoad} -- ${searchPath} -- ${options.paths}`);
        ...
        // add on node_modules
            ...
            debug('loadModule',`checking path ${p2}`);
        ...
        if (finalLocation){
            debug('loadModule',`Loading ${nameToLoad} from ${finalLocation}`);
            const req = require;
            ...
            if (options.errorNotFound && options.errorNotFound===true){
                ...
                debug(`Unable to load ${nameToLoad} from ${searchPath}`);
                return null;
            }
        }
    }
