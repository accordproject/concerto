  ...
  `;
    ...
    describe('#getClassDeclaration', function() {
        it('should return the class declaraction', function () {
            const resource = new Relationship(modelManager, classDecl, 'org.acme.l1@1.0.0', 'Person', '123' );
    ...
    describe('#toJSON', () => {
        it('should throw is toJSON is called', function () {
            const relationship = new Relationship(modelManager, classDecl, 'org.acme.l1@1.0.0', 'Person', '123' );
            (function () {
                relationship.toJSON();
    ...
    describe('#uri serialization', function() {
        it('check that relationships can be serialized to URI', function() {
            const rel = new Relationship(modelManager, classDecl, 'org.acme.l1@1.0.0', 'Person', '123' );
            rel.toURI().should.equal('resource:org.acme.l1@1.0.0.Person#123');
        });

        it('check that unicode relationships can be serialized to URI', function() {
            let Omega = '\u03A9';
            const rel = new Relationship(modelManager, classDecl, 'org.acme.l1@1.0.0', 'Person', Omega );
            rel.toURI().should.equal('resource:org.acme.l1@1.0.0.Person#%CE%A9');
        });

        it('check that relationships can be created from a URI', function() {
            const rel = Relationship.fromURI(modelManager, 'resource:org.acme.l1@1.0.0.Person#123' );
            rel.getNamespace().should.equal('org.acme.l1@1.0.0');
        ...
        it('check that relationships can be created from a unicode URI', function() {
            const rel = Relationship.fromURI(modelManager, 'resource:org.acme.l1@1.0.0.Person#%CE%A9' );
            rel.getNamespace().should.equal('org.acme.l1@1.0.0');
            rel.getType().should.equal('Person');
            let Omega = '\u03A9';
        ...
        it('check that relationships can be created from a legacy fully qualified identifier', function() {
            const rel = Relationship.fromURI(modelManager, 'org.acme.l1@1.0.0.Person#123' );
            rel.getNamespace().should.equal('org.acme.l1@1.0.0');
            rel.getType().should.equal('Person');
            rel.getIdentifier().should.equal('123');
        });
        ...
        it('legacy fully qualified identifier including tricky characters', function() {
            const rel = Relationship.fromURI(modelManager, 'org.acme.l1@1.0.0.Person#1.2:3#4' );
            rel.getNamespace().should.equal('org.acme.l1@1.0.0');
            rel.getType().should.equal('Person');
        ...
        it('check that relationships can be created from a legacy identifier', function() {
            const rel = Relationship.fromURI(modelManager, '123', 'org.acme.l1@1.0.0', 'Person' );
            rel.getNamespace().should.equal('org.acme.l1@1.0.0');
            rel.getType().should.equal('Person');
            rel.getIdentifier().should.equal('123');
        });
        ...
        it('check invalid name space gets error', function() {
            (function () {
                Relationship.fromURI(modelManager, '123', 'org.acme.empty@1.0.0', 'Person' );
            }).should.throw(/Namespace is not defined for type "org.acme.empty@1.0.0.Person"./);
        });

        it('', function() {
            (function () {
                Relationship.fromURI(modelManager, 'resource:org.acme.l1@1.0.0.Unkown#123' );
            }).should.throw(/Type "Unkown" is not defined in namespace "org.acme.l1@1.0.0"./);
        });

        it('should error on invalid URI scheme', function() {
            (function () {
                Relationship.fromURI(modelManager, 'banana:org.acme.l1@1.0.0.Person#123');
            }).should.throw(/banana/);
        });

        it('should error on invalid URI content', function() {
            (function () {
                Relationship.fromURI(modelManager, 'resource://NOT-A-URI:SUCH-WRONG/org.acme.l1@1.0.0.Person#123');
        ...
        it('should error on URI content that Composer does not support', function() {
            (function () {
                Relationship.fromURI(modelManager, 'resource://USER:PASSWORD@HOSTNAME:1567/org.acme.l1@1.0.0.Person#123');
        ...
        it('should error on missing type in URI', function() {
            (function () {
                Relationship.fromURI(modelManager, 'resource:org.acme.l1@1.0.0.#123');
            }).should.throw();
        });

        it('should error on missing ID', function() {
            (function () {
                Relationship.fromURI(modelManager, '', 'org.acme.l1@1.0.0', 'Person');
            }).should.throw();
        });
    });
